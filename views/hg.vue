  ...
  <div class="app">
    <list ref="list" class="home" show-scrollbar="false" :style="{height:pageHeight+'px'}" @loadmore="loadmore" loadmoreoffset="500" offset-accuracy="30">
      ...
      <cell>
        ...
        <!-- 活动图片1区 -->
      <!-- <cell>
        <div class="activity1" v-if="activityList[0]" @click="openUrl(activityList[0])">
          <image v-if="activityList[0]" :src="activityList[0].pic_url + '_.webp'" class="activity1Img" resize="cover" />
        </div>
      </cell> -->
        <!-- 功能区 -->
      <cell>
        <scroller show-scrollbar="false" scroll-direction="horizontal" class="menu-list row" ref="scrollView" @scroll="onScroll">
          <div class="menu-sell" @click="openUrl(item)" v-for="item in funList">
            <image class="menu-img" :src="item.pic_url+'_120x120q80_.webp'" />
            <text class="menu-text">{{item.remark[0]}}</text>
          </div>
        </scroller>
        <div class="reveal" v-if="funList.length>5?true:false">
        ...
        <!-- 活动图片2区 -->
      <!-- <cell>
        <div class="activity2" v-if="activityList[1]?true:false" @click="openActivityTwo()">
          <image v-if="activityList[1]" :src="activityList[1].pic_url + '_.webp'" class="activity2Img" resize="cover" />
        </div>
      </cell> -->
        <!-- 淘抢购区 -->
      <cell>
      ...
      <cell>
        <div class="con">
          <div class="big-con" @click="openUrl(actionList[0][0])">
            <div class="scrbox row">
              <image class="imglt" :src="cabbageSale" />
              <text class="con-text">白菜价</text>
            </div>
            <text class="com-text">特卖好品 件件低价</text>
            <image class="imgl" v-if="actionList[0][0]" :src="actionList[0][0].pic_url+'_150x150q80_.webp'" />
            <image class="imgl1" v-if="actionList[0][1]" :src="actionList[0][1].pic_url+'_150x150q80_.webp'" />
          </div>
          <div class="big-con" @click="openUrl(actionList[1][0])">
            <div class="scrbox row">
              <image class="imglt" :src="hotSale" />
              <text class="con-text">热销排行榜</text>
            </div>
            <text class="com-text">最低0元起</text>
            <image class="imgl" v-if="actionList[1][0]" :src="actionList[1][0].pic_url+'_150x150q80_.webp'" />
            <image class="imgl1" v-if="actionList[1][1]" :src="actionList[1][1].pic_url+'_150x150q80_.webp'" />
          </div>
          <div class="big-con" @click="openUrl(actionList[2])">
            <div class="scrbox row">
              <image class="imglt" :src="jimiSale" />
              <text class="con-text">积米零食</text>
            </div>
            <image class="imgl3" v-if="actionList[2]" :src="actionList[2].pic_url+'_q90_.webp'" resize="contain" />
          </div>
          <div class="big-con" @click="openUrl(actionList[3])">
            <div class="scrbox row">
              <image style="width:100px;height:30px;margin-right: 5px;" :src="fashionSale" />
              <text class="con-text">潮流范</text>
            </div>
            <image class="imgl3" v-if="actionList[3]" :src="actionList[3].pic_url+'_q90_.webp'" resize="contain" />
          </div>
        </div>
      </cell>
        <!-- 精品商品区 -->
      ...
      <!-- <cell class="small-patch" v-for="(goods,index) in dataList.goodList" v-if="index % 2 == 0" :key="goods.item_id">
        <div v-for="(i,idx) in 2" :class="[idx == 0 ? 'left' : 'right']" @click="toDetails(dataList.goodList[index+idx])">
          <image class="patch-img" :src="dataList.goodList[index+idx].pict_url+'_360x360q80.jpg_.webp'" resize="contain" style="width:360px;height:360px" />
          <div class="intro">
            <div class="title-con">
              <image v-if="dataList.goodList[index+idx].mallIcon" :src="dataList.goodList[index+idx].mallIcon" class="mall-icon" resize="contain"></image>
              <text class="intro-txt">&emsp;&emsp;&ensp;{{dataList.goodList[index+idx].title}}</text>
            </div>
            <div class="case">
              <text class="hotbox-oldpr">原价 ￥{{dataList.goodList[index+idx].zk_final_price}}</text>
              <div class="coupon">
                <text class="sell-txxt" style="">{{dataList.goodList[index+idx].coupon_info | getCoupon}}元券</text>
              </div>
            </div>
            <div class="case">
              <div class="cbse-row">
                <text class="hotbox-symbol">￥</text>
                <text class="hotbox-number">{{dataList.goodList[index+idx].coupon_info,dataList.goodList[index+idx].zk_final_price | getMoney | keepInteger}}</text>
                <text class="hotbox-number decimal">{{dataList.goodList[index+idx].coupon_info,dataList.goodList[index+idx].zk_final_price | getMoney | keepDecimal}}</text>
              </div>
              <div class="sell">
                <text class="sell-txt">售:{{dataList.goodList[index+idx].volume}}</text>
              </div>
            </div>
          </div>
        </div>
      </cell> -->
      <!-- <cell class="cell" v-for="(item,index) in qualityGoodsList.length" :key="index" v-if="index % 2 == 0">
        <list-biserial v-if="qualityGoodsList.length" :list="qualityGoodsList" :num="index" @select="goods => select(goods)"></list-biserial>
      </cell> -->
      <cell>
...
export default {
  mixins: [Util],
  components: { WxcCountdown, WxcIcon },
  ...
  methods: {
    ...
    getData() {
      ...
      // }
      // this.post('Top','hsIndex',param,res => {
      //   console.log('首页',res)
      //   if (res.code === ERR_OK) {
      //     this.dataList = res.data     // bannerList activityList actionList
      //     this.dataList.bannerList.forEach(item => {
      //       switch (item.module) {
      //         case '1':
      //           self['bannerList'].push(item)
      //           break;
      //         case '2':
      //           Vue.set(self['activityList'], 0, item)
      //           break;
      //         case '3':
      //           self['funList'].push(item)
      //           break;
      //         case '4':
      //           Vue.set(self['activityList'], 1, item)
      //           break;
      //         case '5':
      //           // Vue.set(self['actionList'],0,item)
      //           self['actionList'][0].push(item)
      //           break;
      //         case '6':
      //           // Vue.set(self['actionList'],1,item)
      //           self['actionList'][1].push(item)
      //           break;
      //         case '7':
      //           Vue.set(self['actionList'], 2, item)
      //           break;
      //         case '8':
      //           Vue.set(self['actionList'], 3, item)
      //           break;
      //         default:
      //           break;
      //       }
      //     })
      //     this.dataList.goodList.uatm_tbk_item.forEach(item => {
      //       item.mallIcon = item.user_type == '1' ? icons.mallType[0] : icons.mallType[1]
      //     })
      //     //console.log("aaa",self.dataList.timeLimitList)
      //     //this.dataList.LimitList= [...self.dataList.timeLimitList.start, ...self.dataList.timeLimitList.noStart]
      //     if (self.dataList.timeLimitList.start.results == null && self.dataList.timeLimitList.noStart.results == null) {
      //       this.dataList.LimitList = [];
      //     } else if (self.dataList.timeLimitList.start.results == null) {
      //       this.dataList.LimitList = self.dataList.timeLimitList.noStart.results;
      //     } else if (self.dataList.timeLimitList.noStart.results == null) {
      //       this.dataList.LimitList = self.dataList.timeLimitList.start.results;
      //     } else {
      //       this.dataList.LimitList = self.dataList.timeLimitList.start.results.concat(self.dataList.timeLimitList.noStart.results);
      //     }
      //     // if(this.funList.length>0){
      //     //   this.funList.pop();
      //     // }
      //     // console.log('',this.dataList.LimitList)
      //     this.funList.forEach(item => {
      //       item.remark = item.remark.split(',');
      //     })
      //     // console.log('fun', this.funList)
      ...
      this.post('Top','hsIndex',{access_token:this.access_token}, res => {
        ...
        if (res.code === ERR_OK) {
          ...
          console.log(this.qualityGoodsList)
        }
      })
    },
    getBoutique() {
